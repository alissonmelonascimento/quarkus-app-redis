...
public class EmployeeHashService {
    ...
    public void insert(Employee employee) throws JsonProcessingException{
        redisClient.hset(Arrays.asList("employee-"+employee.id, "id", employee.id));
        redisClient.hset(Arrays.asList("employee-"+employee.id, "name", employee.name));
        redisClient.hset(Arrays.asList("employee-"+employee.id, "salary", employee.salary.toString()));
    }
    ...
    public Employee get(String key) throws JsonMappingException, JsonProcessingException {
        ...
        String str = redisClient.hgetall(key).toString();
